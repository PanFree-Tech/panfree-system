/**
 * 📁 UBICACIÓN: src/app/admin/analytics/components/AnalyticsView.jsx
 * 📌 COMPONENTE: Panel de Analítica y Rendimiento - Independiente de Marketing
 ...
 * ✅ MOVIDO de Marketing a /admin/analytics (sección independiente)
 */

'use client'

import { useState, useEffect } from 'react'
import styles from '../../marketing/styles/marketing.module.css'
import ScheduledPosts from '../../marketing/components/ScheduledPosts'
...
export default function AnalyticsView({ refreshTrigger }) {
  ...
  const cargarAnaliticas = async () => {
    try {
      ...
      console.warn('Error al cargar analíticas:', err.message)
      setNotificacion({
        tipo: 'error',
        texto: err.message,
      })
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#334c2b', margin: 0 }}>
            📊 Google Analytics 4 - Rendimiento y Conversiones
          </h2>
          <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.82rem', color: '#666' }}>
            Métricas de conversión, engagement, reglas de descuento y publicaciones.
          </p>
        </div>
